...
- [x] Create `MealHistoryPage` accessible from the HISTÓRICO bottom nav tab
- [x] Date navigation: previous/next day chevrons + date input, default to today
- [x] Query `meal_entries` filtered by `consumed_at` date range, expand `food` relation to get descriptions
- [x] Group entries by `meal_type`, show daily totals (sum of energy_kcal, protein_g, carbohydrate_g, lipid_g, fiber_g)
- [x] Allow editing an existing meal entry — open `AddMealModal` pre-filled with the entry's data
- [x] Allow deleting a meal entry with Quasar confirmation dialog
...
- [x] Install ApexCharts (`apexcharts`) and Vue wrapper (`vue3-apexcharts`) in Quasar project
- [x] Create `boot/apexcharts.js` to register the Vue plugin
- [x] Build `NutritionChart` component using ApexCharts area chart
- [x] Add **SEMANA** (last 7 days) and **MÊS** (last 30 days) toggle buttons
- [x] Query `meal_entries` grouped by date, sum nutrition fields per day
- [x] Add macro toggle buttons: **CALORIAS**, **PROTEÍNA**, **CARBOS**, **GORDURA** — clicking switches the chart's y-axis data series
- [x] X-axis shows dates, Y-axis shows the selected metric value
...
- [x] Build `PhotoUpload` component with two options: **TIRAR FOTO** (camera capture via `navigator.mediaDevices`) and **ESCOLHER IMAGEM** (file picker)
- [x] Add MANUAL/FOTO tabs to `AddMealModal` — MANUAL shows the food search form, FOTO shows the photo upload
- [x] Photo flow: user captures/selects image → frontend sends image to `POST /api/meals/parse` (multipart form data) → PocketBase JS hook (`pb_hooks/meal-parser.pb.js`) calls Mimo V2.5 via OpenCode Go → hook returns structured JSON with recognized foods and estimated portions → frontend populates the meal item list with the results for user review before saving
- [x] Map recognized food names to `foods` collection entries via description search (accent-insensitive)
- [x] Allow adding multiple foods manually in the MANUAL tab (same item row behavior as FOTO results)
- [x] Limit manual food list to 5 items per meal
- [x] Add delete button (×) to each manual item row
- [x] Add CANCELAR button to deselect/clear a selected food before adding it to the list
